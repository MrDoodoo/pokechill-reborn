...
function tooltipData(category, ttdata) {
  ...
  if (category == "trainer") {
    ...
    document.getElementById("tooltipBottom").style.display = "inline";
    document.getElementById("tooltipBottom").innerHTML =
      `<div id="area-preview-spawns"><strong>Team Preview</strong></div><div id="preview-field-effects"></div>`;
    ...
    if (areas[ttdata].itemReward) {
      //new reward function ill eventually have to convert the rest of the code but for now im quite lazy
    ...
    if (areas[ttdata].fieldEffect && areas[ttdata].type != "frontier") {
      document.getElementById("preview-field-effects").innerHTML =
        `Field Effects`;
      document.getElementById("preview-field-effects").className =
        "preview-field-list";
    ...
    if (
      areas[ttdata].type == "event" &&
      (areas[ttdata].difficulty == tier3difficulty ||
        areas[ttdata].difficulty == tier4difficulty)
    ) {
      let itemToAdd = "";
      if (areas[ttdata].ticketIndex == 3 || areas[ttdata].ticketIndex == 6)
        itemToAdd = item.yellowApricorn.id;
      if (areas[ttdata].ticketIndex == 4 || areas[ttdata].ticketIndex == 7)
        itemToAdd = item.pinkApricorn.id;
      if (areas[ttdata].ticketIndex == 5 || areas[ttdata].ticketIndex == 8)
        itemToAdd = item.greenApricorn.id;
      if (areas[ttdata].ticketIndex == 9 || areas[ttdata].ticketIndex == 12)
        itemToAdd = item.blackApricorn.id;
      if (itemToAdd == "") return;
      const div = document.createElement("div");
      div.dataset.item = itemToAdd;
      let tag = "";
      if (areas[ttdata].difficulty == tier3difficulty)
        tag = `<span>Uncommon!</span>`;
      div.className = "area-preview";
      div.innerHTML =
        `<img style="scale:2" src="img/items/${itemToAdd}.png">` + tag;
      document.getElementById("area-preview-items").appendChild(div);
    }
  }
  ...
  if (category == "buff") {
    ...
    if (ttdata === "embargo")
      document.getElementById("tooltipBottom").innerHTML =
        `Prevents the enemy to use the move in the slot embargo was used`;

    if (ttdata === "sunny")
      document.getElementById("tooltipBottom").innerHTML =
        `Increases the damage of Fire-Type moves by 50% and decreases the damage of Water-Type moves by 50%`;
    if (ttdata === "rainy")
      document.getElementById("tooltipBottom").innerHTML =
        `Increases the damage of Water-Type moves by 50% and decreases the damage of Fire-Type moves by 50%`;
    if (ttdata === "sandstorm")
      document.getElementById("tooltipBottom").innerHTML =
        `Increases the damage of Rock and Ground-Type moves by 50%`;
    if (ttdata === "hail")
      document.getElementById("tooltipBottom").innerHTML =
        `Increases the damage of Ice-Type moves by 50%`;
    if (ttdata === "foggy")
      document.getElementById("tooltipBottom").innerHTML =
        `Increases the damage of Dark and Ghost-Type moves by 50%`;
    if (ttdata === "electricTerrain")
      document.getElementById("tooltipBottom").innerHTML =
        `Increases the damage of Electric and Steel-Type moves by 50%`;
    if (ttdata === "grassyTerrain")
      document.getElementById("tooltipBottom").innerHTML =
        `Increases the damage of Grass and Bug-Type moves by 50%`;
    if (ttdata === "mistyTerrain")
      document.getElementById("tooltipBottom").innerHTML =
        `Increases the damage of Fairy and Psychic-Type moves by 50%`;
    ...
    if (ttdata === "weirdRoom")
      document.getElementById("tooltipBottom").innerHTML =
        `Copies all attack and defense base stats of the enemy`;
    if (ttdata === "crossRoom")
      document.getElementById("tooltipBottom").innerHTML =
        `Increases the damage dealt by cross-power by 30%`;
    //if (ttdata==="reflect") document.getElementById("tooltipBottom").innerHTML = `Decreases the damage dealt by physical attacks by 75%`
    if (ttdata === "lightScreen")
      document.getElementById("tooltipBottom").innerHTML =
  ...
  if (category == "arenaCard") {
    ...
    document.getElementById("tooltipMid").style.display = `inline`;
    document.getElementById("tooltipMid").innerHTML =
      `<div id="arena-card-effect-list"></div>`;
    document.getElementById("tooltipBottom").style.display = `none`;
  ...
  if (category == "help") {
    ...
    if (ttdata === `Spiral`)
      document.getElementById("tooltipTitle").innerHTML = `Battle Tower`;
    if (ttdata === `Spiral`)
      document.getElementById("tooltipBottom").innerHTML =
    ...
    if (ttdata === `BattleFactory`)
      document.getElementById("tooltipTitle").innerHTML = `Battle Factory`;
    if (ttdata === `BattleFactory`)
      document.getElementById("tooltipBottom").innerHTML =
    ...
    if (ttdata === `BattleFactory`)
      document.getElementById("tooltipMid").innerHTML =
        `<div id="area-preview-spawns" data-pkmn="${areas.frontierBattleFactory.icon.id}"><strong>Factory Pokemon</strong><img class="sprite-trim" src="img/pkmn/sprite/${areas.frontierBattleFactory.icon.id}.png"></div>`;
    if (ttdata === `BattleFactory`)
      document.getElementById("tooltipMid").style.display = `inline`;

    if (ttdata === `BattleArena`)
      document.getElementById("tooltipTitle").innerHTML = `Battle Arena`;
    if (ttdata === `BattleArena`)
      document.getElementById("tooltipBottom").innerHTML =
    ...
    if (ttdata === `Dimension`)
      document.getElementById("tooltipTitle").innerHTML = `Mega-Dimension`;
    if (ttdata === `Dimension`)
      document.getElementById("tooltipBottom").innerHTML =
    ...
    if (ttdata === `searchDictionary`)
      document.getElementById("tooltipBottom").innerHTML =
        `Operators:<br>![keyword]: Exclude from search<br>[keywordA] or [keywordB]: Search keywordA OR keywordB<br>[keywordA] [keywordB]: Search for keywordA AND keywordB<br><br>Pokemon keywords:<br>unobtainable, wild, park, event, frontier, mart, shiny, caught, signature, eggMove, [type], [hidden ability]<br><br>Move keywords:<br>physical, special, signature, [type], [ability]`;
    if (ttdata === `searchDictionary`)
    ...
    if (ttdata === `ModHard`)
      document.getElementById("tooltipTitle").innerHTML = `Hard Mode`;
    if (ttdata === `ModHard`)
      document.getElementById("tooltipBottom").innerHTML =
        `Trainers have twice the health. Only applies to VS Trainers`;

    if (ttdata === `ModNuzloke`)
      document.getElementById("tooltipTitle").innerHTML = `Nuzloke Mode`;
    if (ttdata === `ModNuzloke`)
      document.getElementById("tooltipBottom").innerHTML =
        `If any of your Pokemon get defeated while battling a trainer of VS Trainers, it will permanently be unusuable until you turn this mode off`;

    if (ttdata === `ModNoIvs`)
      document.getElementById("tooltipTitle").innerHTML = `No IVs`;
    if (ttdata === `ModNoIvs`)
      document.getElementById("tooltipBottom").innerHTML =
        `All of your Pokemon have functionally max IVs regardless of their actual IVs. This can be toggled off without any permanent changes<br><br>There are no side-effects to this modifier`;

    if (ttdata === `ModAfk`)
      document.getElementById("tooltipTitle").innerHTML = `Longer AFK`;
    if (ttdata === `ModAfk`)
      document.getElementById("tooltipBottom").innerHTML =
        `Fatigue damage is reduced by x3<br><br>There are no side-effects to this modifier`;

    if (ttdata === `curry`)
      document.getElementById("tooltipTitle").innerHTML = `Curry`;
    if (ttdata === `curry`)
      document.getElementById("tooltipBottom").innerHTML =
        `Every 12h you can select up to three abilities to be active for your entire team during raids for 1 hour. These abilities follow the same rules as regular abilities, and wont stack with already existing ones`;
  ...
  if (category == "move") {
    ...
    document.getElementById("tooltipMid").innerHTML +=
      `<svg onclick="tooltipData('dictionaryMove', '${ttdata}')" class="move-tooltip-dictionary" xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24"><path fill="currentColor" d="m6.006 11.823l1.123-3.06h.05l1.123 3.06zm6.494 5.496q1.216-.678 2.453-.98t2.547-.3q.9 0 1.618.111t1.267.296q.23.096.423-.029t.192-.394V7.008q0-.173-.096-.308q-.096-.134-.327-.23q-.825-.293-1.501-.4T17.5 5.961q-1.31 0-2.613.386q-1.304.387-2.387 1.16zm-6.78-4.763h2.869l.474 1.306q.05.111.137.17t.204.059q.198 0 .303-.158t.043-.35L7.739 8.24q-.05-.131-.169-.212t-.255-.081h-.323q-.136 0-.254.08t-.169.213l-2.011 5.361q-.062.173.043.33t.303.159q.117 0 .206-.06q.09-.058.14-.175zm6.28 5.9q-.235 0-.432-.059t-.376-.15q-1.09-.595-2.27-.902T6.5 17.04q-.78 0-1.534.13q-.753.131-1.466.42q-.544.217-1.022-.131T2 16.496V6.831q0-.371.195-.689t.547-.442q.887-.383 1.836-.56T6.5 4.962q1.47 0 2.866.423q1.398.423 2.634 1.23q1.237-.807 2.634-1.23t2.866-.423q.973 0 1.922.178q.95.177 1.836.56q.352.125.547.442t.195.689v9.665q0 .614-.516.943q-.517.328-1.1.111q-.693-.27-1.418-.39q-.724-.121-1.466-.121q-1.24 0-2.421.306t-2.271.901q-.18.093-.376.151q-.197.059-.432.059m1.885-9.508q0-.11.076-.222t.18-.168q.763-.346 1.613-.53q.85-.182 1.746-.182q.48 0 .91.053t.886.153q.129.03.224.135q.096.104.096.257q0 .252-.15.366t-.402.052q-.37-.075-.757-.103q-.388-.028-.807-.028q-.804 0-1.573.154q-.77.154-1.46.43q-.257.099-.42-.005t-.162-.362m0 5.423q0-.11.076-.231q.076-.123.18-.178q.725-.346 1.613-.52q.888-.173 1.746-.173q.48 0 .91.053t.886.153q.129.03.224.135q.096.104.096.257q0 .252-.15.366t-.402.052q-.37-.075-.757-.103q-.388-.028-.807-.028q-.784 0-1.544.16q-.76.161-1.45.457q-.258.118-.44-.003t-.181-.397m0-2.692q0-.11.076-.222t.18-.168q.763-.347 1.613-.53q.85-.182 1.746-.182q.48 0 .91.053t.886.153q.129.03.224.134q.096.104.096.258q0 .252-.15.366t-.402.051q-.37-.075-.757-.102q-.388-.028-.807-.028q-.804 0-1.573.154q-.77.153-1.46.43q-.257.098-.42-.005q-.162-.105-.162-.362"/></svg>`;

    if (
      document.getElementById("pkmn-editor").style.display == "flex" &&
      pkmn[currentEditedPkmn].signature?.id == ttdata
    )
      document.getElementById("tooltipBottom").innerHTML +=
        `<br><br>This is a Signature Move. Signature moves will automatically be learnt at level 100`;
    if (
      document.getElementById("pkmn-editor").style.display == "flex" &&
      pkmn[currentEditedPkmn].eggMove?.id == ttdata
    )
      document.getElementById("tooltipBottom").innerHTML +=
  ...
  if (category == "item") {
    ...
    else if (item[ttdata].type == "memory")
      document.getElementById("tooltipTop").innerHTML =
        `<img src="img/items/${item[ttdata].image}Memory.png">`;
    else if (item[ttdata].type == "decor")
      document.getElementById("tooltipTop").innerHTML =
        `<img src="img/decor/${ttdata}.png" style="scale:2">`;
    else
      document.getElementById("tooltipTop").innerHTML =
  ...
  if (category == "pkmn") {
    ...
    if (pkmn[ttdata].eggMove != undefined)
    ...
    document.getElementById("tooltipMid").innerHTML =
      `<div style="display:flex; flex-wrap:wrap; justify-content:center;align-items:center; overflow-y:scroll; max-height:15rem">${returnTypeMultipliers(pkmn[ttdata])}</div>`;
  ...
  if (category == "pkmnEditor") {
    ...
    document.getElementById("pkmn-editor-name").innerHTML =
      `<span id="pkmn-editor-nickname">${format(poke.id)}</span>${nameTag}`;
    if (pkmn[poke.id].shiny)
      document.getElementById("pkmn-editor-name").innerHTML =
        `<span id="pkmn-editor-nickname">${format(poke.id)}</span> <span style="color:#FF4671">✦</span>${nameTag}`;
    ...
    if (pkmn[currentEditedPkmn].nickname)
    ...
    if (pkmn[ttdata].nature)
      natureText = `<span class="nature-text">${format(pkmn[ttdata].nature)} nature</span>`;
    ...
    if (pkmn[ttdata].signature) {
      ...
      document.getElementById("pkmn-stats-stat-signature").style.display =
        "flex";
      document.getElementById("pkmn-edit-signature-text").textContent =
        "Signature Move";
      ...
      document.getElementById("pkmn-edit-signature-text").textContent =
        "Egg Move";
      document.getElementById("pkmn-edit-signature").innerHTML =
        `${signatureIcon} ${format(pkmn[ttdata].eggMove.id)}`;
      if (pkmn[ttdata].movepool.includes(pkmn[ttdata].eggMove.id))
        document.getElementById("pkmn-edit-signature").innerHTML =
          `${signatureIcon} ${format(pkmn[ttdata].eggMove.id)} ✔️`;
      document.getElementById("pkmn-edit-signature").dataset.move =
        pkmn[ttdata].eggMove.id;
    } else
      document.getElementById("pkmn-stats-stat-signature").style.display =
    ...
    function updateMoves() {
      for (const key in pkmn[ttdata].moves) {
        ...
        if (moveId == pkmn[ttdata].signature?.id)
  ...
  if (category == "dictionaryPkmn") {
    ...
    if (pkmn[ttdata].tagObtainedIn == "mart")
      spawnLocation = `Can be purchased in the Poke-Mart`;
    if (pkmn[ttdata].tagObtainedIn == "arceus")
      spawnLocation = `Unlocked while all obtainable Pokemon have been caught`;
    ...
    let staminaMult = 1;
    if (
      pkmn[ttdata].ability == ability.stamina.id ||
      (pkmn[ttdata].hiddenAbility?.id == ability.stamina.id &&
        pkmn[ttdata].hiddenAbilityUnlocked)
    )
      staminaMult = 2;

    if (saved.gamemodAfk != true)
      document.getElementById("tooltipMid").innerHTML +=
        `This Pokemon can last ${(100 + Math.pow(1.15, 6) * (pkmn[ttdata].bst.hp * 30 + (pkmn[ttdata].bst.def + pkmn[ttdata].bst.sdef) * 15) * staminaMult).toFixed(0)} turns before fainting from Battle Fatigue at maximum IVs`;
    else
      document.getElementById("tooltipMid").innerHTML +=
        `This Pokemon can last ${(100 + Math.pow(1.15, 6) * (pkmn[ttdata].bst.hp * 30 + (pkmn[ttdata].bst.def + pkmn[ttdata].bst.sdef) * 15) * staminaMult).toFixed(0) * 3} turns before fainting from Battle Fatigue at maximum IVs`;
  ...
  if (category == "dictionaryItem") {
    ...
    if (item[ttdata].type == "decor" && item[ttdata].rarity)
      obtainText += `<span>Can be bought in the Poke-Mart</span>`;
