...
function changeTheme() {
  ...
  if (saved.theme == "default" && saved.currentSeason == season.halloween.id)
    theme = `spooky`;
  if (saved.theme == "default" && saved.currentSeason == undefined)
    theme = `dark`;
...
function learnPkmnMove(id, level, mod, exclude = []) {
  ...
  while (attempts++ < MAX_ATTEMPTS) {
    ...
    if (
      ...
      mod == "wild" &&
      move[chosenMove].power == 0
    )
      continue; //no setup moves in training
...
guide.stab = {
  name: `Battle: STAB`,
  description: function () {
    return `If a Pokemon uses a damaging move that has the same type as one of that Pokemon's types, the move's damage is increased by x1.5<br>This is known as same-type attack bonus, or STAB<br><br>Furthermore, Pokemon with a single typing will receive +0.2 extra STAB damage`;
  },
};
...
guide.crossStab = {
  ...
  description: function () {
    return `If a Pokemon uses a damaging move that is preceded (immediately or not) by a damaging move of a different type of the executed move, it will receive a 1.3x damage multiplier. This is indicated with a cross pattern on the move bar of the affected move`;
  },
};
...
guide.buffsDebuffs = {
  ...
  description: function () {
    return `Like status effects, increases or decreases of stats last for 3 turns by default (Except Speed decreases, which last 2)<br><br>Buffs and debuffs of the same magnitude will not stack with each other (IE x2 50% Attack Up), but different ones will (50% + 100% Attack Up)`;
  },
};
...
guide.weather = {
  name: `Battle: Weather`,
  description: function () {
    return `Certain moves can change the weather to altered ones such as ${tagSunny}, ${tagRainy}, ${tagSandstorm}, ${tagHail}, ${tagFoggy}, ${tagElectricTerrain}, ${tagGrassyTerrain}, ${tagMistyTerrain}, ${tagTrickRoom}, ${tagWeirdRoom}, ${tagCrossRoom}, ${tagLightScreen} or ${tagSafeguard}<br><br>You can further check their effects by right click/long press<br><br>Altered weathers will last for 15 turns, and only can be changed after 30`;
  },
};
...
function updateSettings(alt) {
  ...
  if (saved.hideGotPkmn == "true") {
    document.getElementById("settings-hide-got").value = "true";
  } else document.getElementById("settings-hide-got").value = "false";
  if (saved.alternateWildRotation == "true") {
    document.getElementById("settings-alternate-rotation").value = "true";
  } else document.getElementById("settings-alternate-rotation").value = "false";

  if (document.getElementById("tooltip-modifiers-list")) {
    if (alt != true)
      if (saved.gamemodAfk == true) {
        document.getElementById("checkbox-mode-afk").checked = true;
      } else document.getElementById("checkbox-mode-afk").checked = false;
    if (alt == true)
      if (document.getElementById("checkbox-mode-afk").checked) {
        saved.gamemodAfk = true;
      } else saved.gamemodAfk = false;

    if (alt != true)
      if (saved.gamemodHard == true) {
        document.getElementById("checkbox-mode-hard").checked = true;
      } else document.getElementById("checkbox-mode-hard").checked = false;
    if (alt == true)
      if (document.getElementById("checkbox-mode-hard").checked) {
        saved.gamemodHard = true;
      } else saved.gamemodHard = false;

    if (alt != true)
      if (saved.gamemodNuzlocke == true) {
        document.getElementById("checkbox-mode-nuzloke").checked = true;
      } else document.getElementById("checkbox-mode-nuzloke").checked = false;
    if (alt == true)
      if (document.getElementById("checkbox-mode-nuzloke").checked) {
        saved.gamemodNuzlocke = true;
      } else saved.gamemodNuzlocke = false;

    if (alt != true)
      if (saved.gamemodIvs == true) {
        document.getElementById("checkbox-mode-ivs").checked = true;
      } else document.getElementById("checkbox-mode-ivs").checked = false;
    if (alt == true)
      if (document.getElementById("checkbox-mode-ivs").checked) {
        saved.gamemodIvs = true;
      } else saved.gamemodIvs = false;
  }
}
