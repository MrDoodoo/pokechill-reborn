...
function openDecor() {
  ...
  if (pkmn[currentEditedPkmn].decor?.decor != undefined)
    currentDecor = pkmn[currentEditedPkmn].decor.decor;

  document.getElementById("decor-menu").style.display = "flex";

  document.getElementById("pkmn-decor-dress").src =
    `img/pkmn/sprite/${currentDecorPokemon}.png`;
  if (pkmn[currentEditedPkmn].shiny && !pkmn.piloswine.shinyDisabled)
    document.getElementById("pkmn-decor-dress").src =
      `img/pkmn/shiny/${currentDecorPokemon}.png`;
  ...
  if (isOutside) {
    ...
    if (currentDecor != undefined) {
      ...
      document.getElementById("pkmn-edit-decor").src =
        `img/decor/${currentDecor}.png`;
      document.getElementById("pkmn-edit-decor").style =
        `position: absolute; left: ${relativeX}px; top: ${relativeY}px; pointer-events: none;`;
