...
function startPassiveTraining(pkmn) {
  isTrainingPassive = true;
  trainingStartTime = Date.now();
  pkmn.isTraining = true; // On marque ce Pokémon comme "occupé"
  alert(
    `${pkmn.name} commence son entraînement ! Tu peux continuer à voyager.`,
  );
}
...
function setWildPkmn() {
  ...
  if (saved.currentArea == areas.training.id) {
    ...
    document.getElementById(`factory-highest-score`).innerHTML =
    ...
    if (!areas[saved.currentArea].timed)
      document.getElementById(`team-indicator`).style.display = `flex`;
    if (areas[saved.currentArea].timed)
      document.getElementById(`raid-timer-indicator`).style.display = `flex`;
    ...
    if (currentTrainerSlot > maxTrainerSlot) {
      ...
      if (
        ...
        areas[saved.currentArea].tier
      ) {
        saved.arenaCurrentTrainer++;
        createArenaCards();

        if (saved.arenaActiveCard == 1) {
          item.goldenBottleCap.got += 2;
          item.goldenBottleCap.newItem += 2;
        }
        if (saved.arenaActiveCard == 2) {
          item.goldenBottleCap.got += 3;
          item.goldenBottleCap.newItem += 3;
        }
        if (saved.arenaActiveCard == 3) {
          item.goldenBottleCap.got += 4;
          item.goldenBottleCap.newItem += 4;
        }

        if (item.battlePass.got > 0) {
          item.goldenBottleCap.got += 3;
          item.goldenBottleCap.newItem += 3;
        }
      }
      ...
      if (areas[saved.currentArea].itemReward) {
        //new function
        ...
        for (const key in rewards) {
          ...
          if (item[rewardId] !== undefined) {
            for (let i = 0; i < amount; i++) {
              ...
              if (
                (item[rewardId].type === "held" &&
                  item[rewardId].got >= 20 &&
                  item[rewardId].heldBonusPkmn === undefined) ||
                (item[rewardId].heldBonusPkmn &&
                  pkmn[item[rewardId].heldBonusPkmn()].caught === 0 &&
                  item[rewardId].got >= 21) ||
                (item[rewardId].heldBonusPkmn &&
                  pkmn[item[rewardId].heldBonusPkmn()].caught > 0 &&
                  item[rewardId].got >= 20)
              ) {
      ...
      //apricorn code
      if (
        areas[saved.currentArea].type == "event" &&
        (areas[saved.currentArea].difficulty == tier3difficulty ||
          areas[saved.currentArea].difficulty == tier4difficulty)
      ) {
        let itemToAdd = "";
        if (
          areas[saved.currentArea].ticketIndex == 3 ||
          areas[saved.currentArea].ticketIndex == 6
        )
          itemToAdd = item.yellowApricorn.id;
        if (
          areas[saved.currentArea].ticketIndex == 4 ||
          areas[saved.currentArea].ticketIndex == 7
        )
          itemToAdd = item.pinkApricorn.id;
        if (
          areas[saved.currentArea].ticketIndex == 5 ||
          areas[saved.currentArea].ticketIndex == 8
        )
          itemToAdd = item.greenApricorn.id;
        if (
          areas[saved.currentArea].ticketIndex == 9 ||
          areas[saved.currentArea].ticketIndex == 12
        )
          itemToAdd = item.blackApricorn.id;
        if (
          (areas[saved.currentArea].difficulty == tier3difficulty &&
            rng(0.2)) ||
          areas[saved.currentArea].difficulty == tier4difficulty
      ...
      if (
        ...
        /move|iv1|iv2|iv3/.test(areas.training.currentTraining) &&
  ...
  if (saved.gamemodHard == true && areas[saved.currentArea].type == "vs")
    hpMultiplier *= 2;

  wildPkmnHp = (100 + hpStars * 30 * (1 + wildLevel * 0.2)) * hpMultiplier;

  if (areas[saved.currentArea].difficulty == tier1difficulty)
    wildPkmnHp = 45250;
  if (areas[saved.currentArea].difficulty == tier2difficulty)
    wildPkmnHp = 139300;
  if (areas[saved.currentArea].difficulty == tier3difficulty)
    wildPkmnHp = 398000;
  if (areas[saved.currentArea].difficulty == tier4difficulty)
    wildPkmnHp = 1302000;
  ...
  for (let index = 0; index < randomMoves.length; index++) {
    ...
    if (pkmn[saved.currentPkmn].eggMove?.id == i)
...
function exitCombat() {
  for (const i in team) {
    ...
    if (pkmn[team[i].pkmn.id].typeTemp && !testAbility(i, ability.protean.id))
      pkmn[team[i].pkmn.id].type = pkmn[team[i].pkmn.id].typeTemp;
...
function leaveCombat() {
  if (areas[saved.currentArea].hpPercentage) {
    ...
    if (
      areas[saved.currentArea].hpPercentage > 0 &&
      areas[saved.currentArea].encounterEffect
    )
      areas[saved.currentArea].encounterEffect();

    //if killed, reset
    if (areas[saved.currentArea].hpPercentage <= 0)
      areas[saved.currentArea].hpPercentage = 100;
  }
  ...
  if (saved.currentArea == areas.frontierSpiralingTower.id) {
    document.getElementById("exit-tower").style.display = "flex";
  } else {
    document.getElementById("exit-tower").style.display = "none";
  }
  ...
  if (battleFactoryScore !== 0 && battleFactoryScore == saved.maxFactoryScore) {
    ...
    if (saved.maxFactoryScore >= minimumScore) {
      const totalRewardsEarned = Math.floor(
        (saved.maxFactoryScore - minimumScore) / scoreThreshold,
      );
      const rewardsToGive =
        totalRewardsEarned - (saved.factoryRewardsClaimed || 0);
      ...
      saved.factoryRewardsClaimed =
        (saved.factoryRewardsClaimed || 0) + rewardsToGive;
    }
  }
  if (saved.currentArea == areas.frontierBattleFactory.id) {
    ...
    if (battleFactoryScore < minimumScore) {
      ...
      const nextRewardThreshold =
        minimumScore + scoreThreshold * (totalRewardsEarned + 1);
  ...
  for (const slot in team) {
    ...
    if (testAbility(slot, ability.goodAsGold.id))
      shinyPkmnChanceEncounter *= 1.15;
  }
    ...
    for (let i = 0; i < item.mysteryEgg.got; i++) {
      ...
      for (const iv in pkmn[hatchedPkmn].ivs) {
        ...
        if (newIv > ivId) {
          ...
          pkmn[hatchedPkmn].dictionaryTagIvSum =
            pkmn[hatchedPkmn].ivs.hp +
            pkmn[hatchedPkmn].ivs.atk +
            pkmn[hatchedPkmn].ivs.satk +
            pkmn[hatchedPkmn].ivs.spe +
            pkmn[hatchedPkmn].ivs.sdef +
            pkmn[hatchedPkmn].ivs.def;
        }
      }
      ...
      if (
        pkmn[hatchedPkmn].shiny == true &&
        giveStarsign(hatchedPkmn, `check`) != "complete" &&
        rng(1 / 4000)
      ) {
        //starsign
  ...
  for (let i = activeBars - 1; i >= 0; i--) {
    ...
    if (percent > start) {
      ...
      if (areas[saved.currentArea]?.skills != undefined) {
        if (
          bar == document.getElementById("exploe-wild-hp-4") &&
          skillEnemyTriggers[1] == false
        ) {
          if (areas[saved.currentArea]?.skills[1] != undefined)
            skill[areas[saved.currentArea].skills[1]].effect();
          skillEnemyTriggers[1] = true;
        }

        if (
          bar == document.getElementById("exploe-wild-hp-3") &&
          skillEnemyTriggers[2] == false
        ) {
          if (areas[saved.currentArea]?.skills[2] != undefined)
            skill[areas[saved.currentArea].skills[2]].effect();
          skillEnemyTriggers[2] = true;
        }

        if (
          bar == document.getElementById("exploe-wild-hp-2") &&
          skillEnemyTriggers[3] == false
        ) {
          if (areas[saved.currentArea]?.skills[3] != undefined)
            skill[areas[saved.currentArea].skills[3]].effect();
...
function openMenu() {
  ...
  } else document.getElementById(`menu-wonder-trade`).style.display = "none";
  ...
  if (areas.vsMasterTrainerGeeta.defeated == false) {
    document.getElementById(`menu-wonder-trade`).className =
      `menu-item menu-item-locked`;
  } else {
    document.getElementById(`menu-wonder-trade`).className = `menu-item`;
  }

  if (areas.vsLegendTrainerBrendan.defeated == false) {
    document.getElementById(`menu-dimension`).className =
      `menu-item menu-item-locked`;
  } else {
    document.getElementById(`menu-dimension`).className = `menu-item`;
  }
...
function updateTeamPkmn() {
  for (const i in team) {
    ...
    if (pkmn[team[exploreActiveMember].pkmn.id].playerHp <= 0) {
      ...
      if (
        saved.gamemodNuzlocke == true &&
        areas[saved.currentArea]?.type == "vs"
      )
        pkmn[team[exploreActiveMember].pkmn.id].nuzlocked = true;

      switchMemberNext();

      for (const slot in team) {
        if (testAbility(slot, ability.powerOfAlchemy.id))
          pkmn[team[slot].pkmn.id].abilityTemp = pkmn[team[i].pkmn.id].ability;
      }
...
function returnStatDotsBase(id, stat) {
  ...
  const max = 6;
  const filled = Array(val)
    .fill("★")
    .map((star) => `<span style="color:#29A1E5">${star}</span>`)
    .join(" ");
  const empty = Array(max - val)
    .fill("·")
    .map((dot) => `<span>${dot}</span>`)
    .join(" ");
...
function returnStatDots(id, stat) {
  ...
  if (natureModifier !== 0 && val > 0) {
    ...
    const regularStars =
      val > 1
        ? Array(val - 1)
            .fill('<span style="color:#29A1E5">★</span>')
            .join("")
        : "";
...
function returnIVDots(id, stat) {
  ...
  const filled = Array(val)
    .fill('<span style="color:#F4607C">★</span>')
    .join("");
  const empty = Array(max - val)
    .fill("<span>·</span>")
    .join("");
...
function exploreCombatPlayer() {
  ...
  if (
    nextMoveBoxPlayer !=
    document.getElementById(
      `pkmn-movebox-slot${currentTurn}-team-${exploreActiveMember}`,
    )
  )
  ...
  if (
    barPlayer !=
    document.getElementById(
      `pkmn-movebox-slot${currentTurn}-team-${exploreActiveMember}-bar`,
    )
  )
  ...
  if (
    ...
    moveTimerPlayer /= 1.5;
  if (
    testAbility(`active`, ability.neuroforce.id) &&
    (move[nextMovePlayer].type == "psychic" ||
      move[nextMovePlayer].type == "fairy")
  )
    moveTimerPlayer /= 1.5;
  ...
  if (
    testAbility(`active`, ability.neuroforce.id) &&
    testAbility(`active`, ability.espilate.id) &&
    move[nextMovePlayer].type == "normal"
  )
    moveTimerPlayer /= 1.5;
  if (
    testAbility(`active`, ability.neuroforce.id) &&
    testAbility(`active`, ability.pixilate.id) &&
    move[nextMovePlayer].type == "normal"
  )
    moveTimerPlayer /= 1.5;

  if (
    testAbility(`active`, ability.cacophony.id) == true &&
    move[nextMovePlayer].affectedBy?.includes(ability.cacophony.id)
  )
    moveTimerPlayer /= 2;
  if (
    testAbility(`active`, ability.cacophony.id) == "nerf" &&
    move[nextMovePlayer].affectedBy?.includes(ability.cacophony.id)
  )
    moveTimerPlayer /= 1.5;

  if (
    testAbility(`active`, ability.dancer.id) == true &&
    move[nextMovePlayer].affectedBy?.includes(ability.dancer.id)
  )
    moveTimerPlayer /= 2;
  if (
    testAbility(`active`, ability.dancer.id) == "nerf" &&
    move[nextMovePlayer].affectedBy?.includes(ability.dancer.id)
  )
    moveTimerPlayer /= 1.5;

  if (
    testAbility(`active`, ability.windRider.id) == true &&
    move[nextMovePlayer].affectedBy?.includes(ability.windRider.id)
  )
    moveTimerPlayer /= 2;
  if (
    testAbility(`active`, ability.windRider.id) == "nerf" &&
    move[nextMovePlayer].affectedBy?.includes(ability.windRider.id)
  )
    moveTimerPlayer /= 1.5;

  if (
    testAbility(`active`, ability.iaido.id) == true &&
    move[nextMovePlayer].affectedBy?.includes(ability.sharpness.id)
  )
    moveTimerPlayer /= 2;
  if (
    testAbility(`active`, ability.iaido.id) == "nerf" &&
    move[nextMovePlayer].affectedBy?.includes(ability.sharpness.id)
  )
    moveTimerPlayer /= 1.5;

  if (
    team[exploreActiveMember].item == item.quickClaw.id &&
    move[nextMovePlayer].affectedBy?.includes(ability.libero.id)
  )
    moveTimerPlayer /= item.quickClaw.power();
  if (
    team[exploreActiveMember].item == item.powerHerb.id &&
    move[nextMovePlayer].power == 0
  )
    moveTimerPlayer /= item.powerHerb.power();

  if (areas[saved.currentArea].fieldEffect?.includes(field.averageTime.id)) {
    moveTimerPlayer = defaultPlayerMoveTimer;
  }
  ...
  if (
    afkSeconds <= 0 &&
    lastCrossStab != undefined &&
    lastCrossStab != move[nextMovePlayer].type &&
    /*pkmn[team[exploreActiveMember].pkmn.id].type.includes(move[nextMovePlayer].type) &&*/ move[
      nextMovePlayer
    ].power > 0 &&
    barPlayer.style.backgroundImage != crossPattern &&
  ...
  if (areas[saved.currentArea].fieldEffect?.includes(field.trickField.id)) {
    speedStars = 7 - speedStars;
  }
  if (saved.weatherTimer > 0 && saved.weather == "trickRoom") {
    speedStars = 7 - speedStars;
  }
  ...
  barProgressPlayer +=
    100 /
    ((moveTimerPlayer *
      (Math.pow(0.9, speedStars) * Math.pow(0.95, speedIvs))) /
      (1000 / 60));
  ...
  if (barProgressPlayer < 99) {
    ...
    if (nextMove.id == move.embargo.id)
      embargoSlot = team[exploreActiveMember].turn - 1;
    ...
    if (
      nextMove.multihit &&
      team[exploreActiveMember].item == item.loadedDice.id
    )
      multihit = nextMove.multihit[1];
    movePower *= multihit;
    ...
    if (
      team[exploreActiveMember].item == item.assaultVest.id &&
      nextMove.power == 0
    )
      nextMove = move.splash;
    ...
    if (nextMove.split == "physical") {
      ...
      if (areas[saved.currentArea].id == areas.training.id)
        attackerStars = returnDivisionStars(attacker, "atk");
      if (saved.weatherTimer > 0 && saved.weather == "weirdRoom")
        attackerStars = Math.max(defender.bst.atk - 2, 1);

      let defenderStars = defender.bst.def;
      if (areas[saved.currentArea].id == areas.training.id)
        defenderStars = returnDivisionStars(defender);
      ...
      totalPower =
        (movePower +
          Math.max(
            0,
            attackerStars * 30 * Math.pow(1.1, attacker.ivs.atk) -
              defenderStars * 30,
          )) *
      ...
      if (saved.gamemodIvs == true) {
        totalPower =
          (movePower +
            Math.max(
              0,
              attackerStars * 30 * Math.pow(1.1, 6) - defenderStars * 30,
            )) *
          (1 + attacker.level * 0.1) *
          1;
      }
    ...
    if (nextMove.split == "special") {
      ...
      if (attacker.nature == "quiet") attackerStars--;
      if (areas[saved.currentArea].id == areas.training.id)
        attackerStars = returnDivisionStars(attacker, "satk");
      if (saved.weatherTimer > 0 && saved.weather == "weirdRoom")
        attackerStars = Math.max(defender.bst.satk - 2, 1);
      ...
      totalPower =
        (movePower +
          Math.max(
            0,
            attackerStars * 30 * Math.pow(1.1, attacker.ivs.satk) -
              defenderStars * 30,
          )) *
        (1 + attacker.level * 0.1) *
        1;
    ...
    if (testAbility(`active`, ability.normalize.id) && moveType !== "normal") {
      moveType = "normal";
      totalPower *= 1.3;
    }
    ...
    if (testAbility(`active`, ability.dragonMaw.id) && moveType == "normal") {
      moveType = "dragon";
      totalPower *= 1.3;
    }
    ...
    if (
      ...
      team[exploreActiveMember].item?.split("Gem")[0] === moveType
    )
      totalPower *= stabBonus; //type gems

    if (
      areas[saved.currentArea].fieldEffect?.includes(field.wonderWard.id) &&
      typeEffectiveness(moveType, pkmn[saved.currentPkmn].type) <= 1
    )
      totalPower *= 0.2;
    ...
    if (
      areas[saved.currentArea].fieldEffect?.includes(field.ironBody.id) &&
      typeEffectiveness(moveType, pkmn[saved.currentPkmn].type) > 1
    )
      typeMultiplier = 1;
    ...
    if (defender.temporalType) {
      if (typeEffectiveness(moveType, defender.temporalType) > 1)
        typeMultiplier *= 1.25;
      if (typeEffectiveness(moveType, defender.temporalType) < 1)
        typeMultiplier *= 0.75;
    }
    ...
    if (
      ...
      pkmn[saved.currentPkmn].type.includes("water")
    )
      typeMultiplier *= 1.5;
    ...
    if (testAbility(`active`, ability.treasureOfRuin.id))
      crossPowerBonus += 0.5;
    ...
    if (
      ...
      if (saved.weatherTimer > 0 && saved.weather == "crossRoom")
        totalPower *= 1.3;
    }
    ...
    if (team[exploreActiveMember].item == item.ejectButton.id)
      totalPower *= item.ejectButton.power();
    if (team[exploreActiveMember].item == item.ejectPack.id)
      totalPower *= item.ejectPack.power();

    if (
      team[exploreActiveMember].item == item.loadedDice.id &&
      nextMove.affectedBy?.includes(ability.skillLink.id)
    )
      totalPower *= item.loadedDice.power();
    if (
      team[exploreActiveMember].item == item.luckyPunch.id &&
      nextMove.affectedBy?.includes(ability.ironFist.id)
    )
      totalPower *= item.luckyPunch.power();
    if (
      team[exploreActiveMember].item == item.metronome.id &&
      nextMove.buildup !== undefined
    )
      totalPower *= item.metronome.power();
    if (
      team[exploreActiveMember].item == item.laggingTail.id &&
      nextMove.affectedBy?.includes(ability.reckless.id)
    )
      totalPower *= item.laggingTail.power();

    if (team[exploreActiveMember].item?.endsWith("Gem"))
      totalPower *= item[team[exploreActiveMember].item].power();
    ...
    if (
      ...
      totalPower *= 1.2;
    if (
      testAbility(`active`, ability.merciless.id) == true &&
      (team[exploreActiveMember].buffs?.burn > 0 ||
        team[exploreActiveMember].buffs?.confused > 0 ||
        team[exploreActiveMember].buffs?.paralysis > 0 ||
        team[exploreActiveMember].buffs?.poisoned > 0 ||
        team[exploreActiveMember].buffs?.frozen > 0 ||
        team[exploreActiveMember].buffs?.sleep > 0 ||
        team[exploreActiveMember].buffs?.embargo > 0)
    )
      totalPower *= 1.5;
    if (
      testAbility(`active`, ability.merciless.id) == "nerf" &&
      (team[exploreActiveMember].buffs?.burn > 0 ||
        team[exploreActiveMember].buffs?.confused > 0 ||
        team[exploreActiveMember].buffs?.paralysis > 0 ||
        team[exploreActiveMember].buffs?.poisoned > 0 ||
    ...
    if (testAbility(`active`, ability.gorillaTactics.id) == true)
      totalPower *= 1.5;
    if (testAbility(`active`, ability.gorillaTactics.id) == "nerf")
      totalPower *= 1.35;
    ...
    for (const member in team) {
      if (team[member].pkmn == undefined) continue;
      if (testAbility(member, ability.soulAsterism.id) && moveType == "ghost")
        totalPower *= 1.1;
      ...
      if (team[member].item && item[team[member].item].zType) {
        ...
        if (zCrystalTurn == item[team[member].item].power()) {
          ...
          if (afkSeconds < 10) {
            const heldItem = document.getElementById(
              `team-${member}-held-item`,
            );
          ...
          if (zSplit == "physical" || (zSplit == "random" && rng(0.5))) {
            zTotalPower =
              (zPower +
                Math.max(
                  0,
                  zUser.bst.atk * 30 * Math.pow(1.1, zUser.ivs.atk) -
                    defender.bst.def * 30,
                )) *
              (1 + zUser.level * 0.1) *
              1;
            ...
            zTotalPower =
              (zPower +
                Math.max(
                  0,
                  zUser.bst.satk * 30 * Math.pow(1.1, zUser.ivs.satk) -
          ...
          if (
            ...
            typeEffectiveness(zTyping, pkmn[saved.currentPkmn].type) <= 1
          )
            zTotalPower *= 0.2;

          let zTypeMultiplier = typeEffectiveness(
            zTyping,
            pkmn[saved.currentPkmn].type,
          );
          if (
          ...
          if (defender.temporalType) {
            if (typeEffectiveness(zTyping, defender.temporalType) > 1)
              zTypeMultiplier *= 1.25;
            if (typeEffectiveness(zTyping, defender.temporalType) < 1)
              zTypeMultiplier *= 0.75;
          }

          zTotalPower *= zTypeMultiplier;

          if (
            team[member].damageDealt == undefined ||
            team[member].damageDealt == NaN
          )
            team[member].damageDealt = 0;
          ...
          if (afkSeconds < 10) {
            console.info(
              `Z-move executed: Dealt ${zTotalPower.toFixed(0)} damage`,
            );
            if (zUser.type.includes(zTyping))
              console.info(`- Z-move stab multiplier: Yes (x1.5)`);
            if (zSplit == `random`)
              console.info(`- Z-move random split bonus: Yes (x1.25)`);
    ...
    if (saved.weatherTimer > 0 && saved.weather == "trickRoom")
      totalPower *= Math.pow(
        1.07,
        7 - pkmn[team[exploreActiveMember].pkmn.id].bst.spe,
      );
    ...
    //fieldEffects
    if (
      areas[saved.currentArea].fieldEffect?.includes(field.serendipity.id) &&
      nextMove.hitEffect
    ) {
    ...
    if (
      areas[saved.currentArea].fieldEffect?.includes(field.heavyWeather.id) &&
      saved.weatherTimer > 0
    ) {
      if (saved.weather == "sunny" && !attacker.type.includes("fire"))
        attacker.playerHp -= attacker.playerHpMax / 15;
      if (saved.weather == "rainy" && !attacker.type.includes("water"))
        attacker.playerHp -= attacker.playerHpMax / 15;
      if (
        saved.weather == "sandstorm" &&
        !attacker.type.includes("rock") &&
        !attacker.type.includes("ground")
      )
        attacker.playerHp -= attacker.playerHpMax / 15;
      if (saved.weather == "hail" && !attacker.type.includes("ice"))
        attacker.playerHp -= attacker.playerHpMax / 15;
      if (
        saved.weather == "foggy" &&
        !attacker.type.includes("ghost") &&
        !attacker.type.includes("dark")
      )
        attacker.playerHp -= attacker.playerHpMax / 15;
      if (
        saved.weather == "electricTerrain" &&
        !attacker.type.includes("electric") &&
        !attacker.type.includes("steel")
      )
        attacker.playerHp -= attacker.playerHpMax / 15;
      if (
        saved.weather == "mistyTerrain" &&
        !attacker.type.includes("fairy") &&
        !attacker.type.includes("psychic")
      )
        attacker.playerHp -= attacker.playerHpMax / 15;
      if (
        saved.weather == "grassyTerrain" &&
        !attacker.type.includes("grass") &&
        !attacker.type.includes("bug")
      )
        attacker.playerHp -= attacker.playerHpMax / 15;
    }

    if (
      areas[saved.currentArea].fieldEffect?.includes(field.weakeningCurse.id) &&
      nextMove.split == "physical"
    )
      totalPower *= 0.5;
    if (
      areas[saved.currentArea].fieldEffect?.includes(field.fatiguingCurse.id) &&
      nextMove.split == "special"
    )
      totalPower *= 0.5;
    ...
    if (saved.currentArea == areas.frontierBattleFactory.id) {
      ...
      document.getElementById(`factory-current-score`).innerHTML =
        `Score: ${battleFactoryScore}`;
    }
    ...
    if (team[exploreActiveMember].item == item.lifeOrb.id) {
      attacker.playerHp -= attacker.playerHpMax / 12;
    }
    ...
    if (team[exploreActiveMember].item == item.leftovers.id)
      fatigueDamage /= item.leftovers.power();
    ...
    if (saved.currentArea == areas.frontierBattleFactory.id)
      fatigueDamage = attacker.playerHpMax / 15;
...
function exploreCombatWild() {
  ...
  if (areas[saved.currentArea].fieldEffect?.includes(field.averageTime.id)) {
    moveTimerWild = defaultPlayerMoveTimer;
  }
  ...
  if (barProgressWild < 99) {
    ...
    if (move[nextMoveWild].split == "physical") {
      ...
      if (areas[saved.currentArea].id == areas.training.id)
        attackerStars = returnDivisionStars(pkmn[saved.currentPkmn]);
      ...
      if (pkmn[team[exploreActiveMember].pkmn.id].nature == "jolly")
        defenderStars--;
      if (pkmn[team[exploreActiveMember].pkmn.id].nature == "bold")
        defenderStars++;
      if (areas[saved.currentArea].id == areas.training.id)
        defenderStars = returnDivisionStars(pkmn[saved.currentPkmn]);
      if (saved.weatherTimer > 0 && saved.weather == "weirdRoom")
        defenderStars = Math.max(pkmn[saved.currentPkmn].bst.def - 2, 1);
      ...
      if (saved.gamemodIvs == true) {
        totalPower =
          (move[nextMoveWild].power +
            Math.max(
              0,
              attackerStars * 30 - defenderStars * 30 * Math.pow(1.1, 6),
            )) *
          (1 + wildLevel * 0.1) *
          1;
      }
    ...
    if (move[nextMoveWild].split == "special") {
      ...
      if (areas[saved.currentArea].id == areas.training.id)
        defenderStars = returnDivisionStars(pkmn[saved.currentPkmn]);
      if (saved.weatherTimer > 0 && saved.weather == "weirdRoom")
        defenderStars = Math.max(pkmn[saved.currentPkmn].bst.sdef - 2, 1);
    ...
    if (
      typeMultiplier > 1 &&
      move[nextMoveWild].power > 0 &&
      testAbility(`active`, ability.angerPoint.id)
    )
      moveBuff("wild", "atkup2", "self");
    if (
      typeMultiplier > 1 &&
      move[nextMoveWild].power > 0 &&
      testAbility(`active`, ability.justified.id)
    )
      moveBuff("wild", "satkup2", "self");
    if (
      typeMultiplier > 1 &&
      move[nextMoveWild].power > 0 &&
      team[exploreActiveMember].item == item.weaknessPolicy.id
    )
      moveBuff("wild", "speup1", "self", 8);

    if (
      saved.weatherTimer > 0 &&
      saved.weather == "lightScreen" &&
      typeMultiplier > 1
    )
      typeMultiplier = 1;
    if (testAbility(`active`, ability.shieldsDown.id) && typeMultiplier > 1)
      typeMultiplier = 1;
    ...
    if (team[exploreActiveMember].item == item.assaultVest.id)
      totalPower /= item.assaultVest.power();

    if (team[exploreActiveMember].item == item.mentalHerb.id)
      totalPower /= item.mentalHerb.power();
    if (team[exploreActiveMember].item == item.heavyDutyBoots.id)
      totalPower /= item.heavyDutyBoots.power();
    ...
    if (testAbility(`active`, ability.static.id) && rng(0.15))
      moveBuff("wild", "paralysis");
    if (testAbility(`active`, ability.flameBody.id) && rng(0.15))
      moveBuff("wild", "burn");
    if (testAbility(`active`, ability.poisonPoint.id) && rng(0.15))
      moveBuff("wild", "poisoned");
    if (testAbility(`active`, ability.strangeCharm.id) && rng(0.15))
    ...
    for (const slot in team) {
      if (
        testAbility(slot, ability.purifyingSalt.id) &&
        move[nextMoveWild].type == "ghost"
      )
        totalPower /= 1.25;
    }
    ...
    if (saved.weatherTimer > 0 && saved.weather == "trickRoom")
      totalPower *= Math.pow(1.07, 7 - pkmn[saved.currentPkmn].bst.spe);

    if (testAbility(`active`, ability.wonderGuard.id) && typeMultiplier <= 1)
      totalPower *= 0.2;
    ...
    if (
      areas[saved.currentArea]?.difficulty >= tier4difficulty &&
      areas[saved.currentArea]?.type == "dimension"
    )
      dotDamage = 200;
    if (
      areas[saved.currentArea]?.difficulty >= tier4difficulty &&
      areas[saved.currentArea]?.type == "dimension" &&
      (areas[saved.currentArea]?.tier == 3 ||
        areas[saved.currentArea]?.tier == 4)
    )
      dotDamage = 400;

    if (wildBuffs.burn > 0 && testAbility("active", ability.scorch.id) == true)
      dotDamage /= 2;
    if (
      wildBuffs.burn > 0 &&
      testAbility("active", ability.scorch.id) == "nerf"
    )
      dotDamage /= 1.5;

    if (
      wildBuffs.poisoned > 0 &&
      testAbility("active", ability.corrosion.id) == true
    )
      dotDamage /= 2;
    if (
      wildBuffs.poisoned > 0 &&
      testAbility("active", ability.corrosion.id) == "nerf"
    )
      dotDamage /= 1.5;
...
function initialiseArea() {
  ...
  for (const slot in team) {
    if (team[slot].pkmn == undefined) continue;
    if (testAbility(slot, ability.powerOfAlchemy.id))
      pkmn[team[slot].pkmn.id].abilityTemp = undefined;
  ...
  if (
    (saved.autoRefight == true && areas[saved.currentArea].encounter) ||
    (saved.autoRefight == true && saved.currentArea == areas.training.id)
  ) {
    document.getElementById("auto-refight-info").style.display = "flex";
    document.getElementById("auto-refight-info").innerHTML = `
    <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24"><path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 3c4.97 0 9 4.03 9 9"><animateTransform attributeName="transform" dur="1.5s" repeatCount="indefinite" type="rotate" values="0 12 12;360 12 12"/></path></svg>
  ...
  if (areas[saved.currentArea].timed)
    raidTimer = areas[saved.currentArea].timed;
}
...
function setWildAreas() {
  ...
  for (const i in season) {
    ...
    if (saved.temporalSeason != undefined)
      seasonTimer = `Limited Area Until ${saved.temporalSeason.end.month}/${saved.temporalSeason.end.day}`;
  ...
  for (const i in areas) {
    ...
    if (
      ...
      divAreas.addEventListener("click", (e) => {
        ...
        if (
          areas[i].encounter &&
          areas[i].difficulty === tier4difficulty &&
          areas.vsUltraEntityLusamine.defeated != true
        )
          return;
        saved.currentAreaBuffer = i;
        document.getElementById(`preview-team-exit`).style.display = "flex";
    ...
    if (areas[i].encounter && areas[i].difficulty === tier4difficulty)
      eventTag = `<strong style="filter:hue-rotate(100deg)" class="event-tag">Tier IV Raid ❖</strong>`;
...
function getSeed() {
  if (areas.vsUltraEntityLusamine.defeated) {
    rotationFrontierMax = 4;
  } else {
    rotationFrontierMax = 3;
  }
...
function updateRaidTimer() {
  ...
  document.getElementById("raid-timer-indicator-current").innerHTML =
...
document.getElementById("pokedex-search").addEventListener("keydown", (e) => {
  if (e.key === "Enter") {
    ...
    allTerms.forEach((term) => {
      ...
      const numericMatch = term.match(
        /^(ivsum|dictionaryTagIvSum)([><=]+)(\d+)$/i,
      );
      if (numericMatch) {
        numericFilters.push({
    ...
    if (results.length > 0 && includeTerms.length > 0) {
      ...
      for (let i = 0; i < sampleSize; i++) {
        ...
        includeTerms.forEach((term) => {
          ...
          if (item.ability && item.ability.toLowerCase().includes(lowerTerm))
            hasAbilityMatch = true;
          if (
            item.hiddenAbility &&
            item.hiddenAbility.id &&
            item.hiddenAbility.id.toLowerCase().includes(lowerTerm)
          )
            hasAbilityMatch = true;
          if (
            item.movepool &&
            item.movepool.some((move) => move.toLowerCase().includes(lowerTerm))
          )
            hasMoveMatch = true;
        });
      }
    ...
    if (shouldExpandFamilies) {
      results.forEach((result) => {
        ...
        family.forEach((member) => {
          ...
          const memberInIndex = fusePkmn
            .getIndex()
            .docs.find((doc) => doc === member);
          if (memberInIndex) {
...
function updatePokedex() {
  ...
  if (memoryToTeach != undefined) {
    ...
    document.getElementById("pokedex-filters-cancel").style.display = "flex";
    document.getElementById("pokedex-filters-title").innerHTML =
      `Select a Pokemon to teach ${format(memoryToTeach)}`;
  }
  ...
  for (const i in pkmn) {
    ...
    if (
      ...
      continue;
    if (
      document.getElementById(`pokedex-filter-signature`).value == "egg" &&
      pkmn[i].eggMove == undefined
    )
      continue;

    if (
      document.getElementById(`pokedex-filter-shiny`).value == "true" &&
      pkmn[i].shiny != true
    )
      continue;
    if (
      document.getElementById(`pokedex-filter-shiny`).value == "false" &&
      pkmn[i].shiny == true
    )
      continue;
    if (
      document.getElementById(`pokedex-filter-shiny`).value == "sign" &&
      (pkmn[i].starsignList == undefined ||
        pkmn[i].shiny != true ||
        giveStarsign(i, "check") == "complete")
    )
      continue;
    if (
      document.getElementById(`pokedex-filter-shiny`).value == "signall" &&
      giveStarsign(i, "check") != "complete"
    )
      continue;
    ...
    if (tagSystemTagSearch.length > 0) {
      ...
      const hasMatchingTag = pkmn[i].tagList.some((pkmnTag) =>
        tagSystemTagSearch.some(
          (searchTag) =>
            pkmnTag.name === searchTag.name &&
            pkmnTag.color === searchTag.color,
        ),
      );
    ...
    if (pkmn[i].caught == 0 && pkmn[i].tagObtainedIn == "unobtainable")
      continue;
  ...
  for (const p of sortedPokemon) {
    ...
    if (pkmn[i].starsign) {
      ...
      if (pkmn[i].shiny)
        div.innerHTML = `<span style="display:flex; white-space:nowrap">lvl ${pkmn[i].level} ${nameMarks}</span> <img style="filter:hue-rotate(${starsign[pkmn[i].starsign].hue}deg)" class="sprite-trim" src="img/pkmn/shiny/${i}.png">`;
      if (pkmn[i].shiny && pkmn[i].shinyDisabled == true)
        div.innerHTML = `<span style="display:flex; white-space:nowrap">lvl ${pkmn[i].level} ${nameMarks}</span><img style="filter:hue-rotate(${starsign[pkmn[i].starsign].hue}deg)" class="sprite-trim" src="img/pkmn/sprite/${i}.png">`;
    }
    ...
    if (memoryToTeach != undefined) {
      //filter pokemon by being able to learn
      if (
        item[memoryToTeach + "Memory"].typings == undefined &&
        !ability[memoryToTeach].type.includes("all") &&
        !ability[memoryToTeach].type.some((t) => pkmn[i].type.includes(t))
      )
        continue;
      if (
        item[memoryToTeach + "Memory"].typings != undefined &&
        !item[memoryToTeach + "Memory"].typings.some((t) =>
          pkmn[i].type.includes(t),
        )
      )
        continue;
    ...
    if (itemToUse != undefined) {
      if (itemToUse == item.rareCandy.id) {
        ...
        div.addEventListener("click", (e) => {
          ...
          if (learntMove != undefined) {
            ...
            if (pkmn[i].evolve && pkmn[i].evolve()[1].level > 0) {
              // if it evolves by level up
              if (
                pkmn[i].level >= pkmn[i].evolve()[1].level &&
                pkmn[pkmn[i].evolve()[1].pkmn.id].caught === 0
              ) {
      ...
      if (itemToUse == item.heartScale.id) {
        if (
          pkmn[i].movepoolMemory == undefined ||
          pkmn[i].movepoolMemory.length == 0
        )
          continue;
...
function tagMenuAssign() {
  ...
  saved.tagSystemTags.forEach((i, index) => {
    ...
    const isActive = pkmn[currentEditedPkmn].tagList.some(
      (tag) => tag.name === i.name && tag.color === i.color,
    );
    div.className = isActive
      ? `tag-system-tag`
      : `tag-system-tag tag-system-inactive`;
    ...
    div.addEventListener("click", (event) => {
      ...
      if (div.className === `tag-system-tag tag-system-inactive`) {
        ...
        const alreadyExists = pkmn[currentEditedPkmn].tagList.some(
          (tag) => tag.name === i.name && tag.color === i.color,
        );
        if (!alreadyExists) {
        ...
        const tagIndex = pkmn[currentEditedPkmn].tagList.findIndex(
          (tag) => tag.name === i.name && tag.color === i.color,
        );
        if (tagIndex > -1) {
...
function updateEditorTags() {
  if (pkmn[currentEditedPkmn].tagList?.length > 0) {
    ...
    document.getElementById(`tag-system-editor-tags`).innerHTML =
      `Click here to add tags`; // No tags, show prompt
    return;
  }
  ...
  if (hasMoreTags) {
    ...
    document.getElementById(`tag-system-editor-tags`).innerHTML +=
      `<span style="color:#999"><svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24"><path fill="currentColor" d="M5 10c-1.1 0-2 .9-2 2s.9 2 2 2s2-.9 2-2s-.9-2-2-2m14 0c-1.1 0-2 .9-2 2s.9 2 2 2s2-.9 2-2s-.9-2-2-2m-7 0c-1.1 0-2 .9-2 2s.9 2 2 2s2-.9 2-2s-.9-2-2-2"/></svg></span>`;
  }
}
...
function tagMenuRemove(id) {
  ...
  document.getElementById(`remove-tag-menu`).innerHTML +=
    `<div class="tag-system-delete" onclick="tagMenuRemoveConfirm(${id})">Delete tag</div>`;
}
...
function tagMenuRemoveConfirm(id) {
  ...
  for (const i in pkmn) {
    if (pkmn[i].tagList && pkmn[i].tagList.length > 0) {
      ...
      pkmn[i].tagList = pkmn[i].tagList.filter(
        (tag) =>
...
function tagMenuCreate() {
  ...
  document
    .getElementById("tag-system-tagColor")
    .addEventListener("input", (e) => {
      document.getElementById("tag-system-colorValue").textContent =
        e.target.value;
    });
}
...
const tagSystemIcons = [
  ...
  {
    name: "bookmark",
    svg: '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M19 21l-7-5-7 5V5a2 2 0 0 1 2-2h10a2 2 0 0 1 2 2z"></path></svg>',
  },
  {
    name: "tag",
    svg: '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M20.59 13.41l-7.17 7.17a2 2 0 0 1-2.83 0L2 12V2h10l8.59 8.59a2 2 0 0 1 0 2.82z"></path><line x1="7" y1="7" x2="7.01" y2="7"></line></svg>',
  },
  {
    name: "flag",
  ...
  {
    name: "circle",
    svg: '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="10"></circle></svg>',
  },
  {
    name: "square",
    svg: '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><rect x="3" y="3" width="18" height="18" rx="2" ry="2"></rect></svg>',
  },
  {
    name: "check",
  ...
  {
    name: "folder",
    svg: '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M22 19a2 2 0 0 1-2 2H4a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h5l2 3h9a2 2 0 0 1 2 2z"></path></svg>',
  },
  {
    name: "music",
...
function tagSystemSelectIcon(index) {
  document
    .querySelectorAll(".tag-system-icon-option")
    .forEach((el) => el.classList.remove("tag-system-selected"));
  document
    .querySelectorAll(".tag-system-icon-option")
    [index].classList.add("tag-system-selected");
...
function tagSystemRenderTags() {
  ...
  if (saved.tagSystemTags.length === 0) {
    tagsList.innerHTML =
      '<div class="tag-system-empty-state">No tags created yet. Create your first tag above!</div>';
    return;
  }

  tagsList.innerHTML = saved.tagSystemTags
    .map(
      (tag) => `
...
function switchMenu(id) {
  ...
  if (id == "dimension" && areas.vsLegendTrainerBrendan.defeated == false) {
    ...
    document.getElementById("tooltipBottom").style.display = `none`;
    document.getElementById("tooltipMid").innerHTML =
      `Defeat Legend Trainer Brendan in VS mode to unlock`;
    openTooltip();
    return;
  }
  ...
  if (id === "gauntlet") {
    saved.currentArea = "gauntlet";
    document.getElementById("content-explore").style.display = "flex";
    initialiseArea();

    // On force le lancement d'un combat immédiatement
    if (typeof setWildPkmn === "function") {
      setWildPkmn();
    }
    return;
  }
...
function updateItemBag() {
  ...
  for (const i in item) {
    if (
      !item[i].type?.includes(bagCategory) &&
      (bagCategory != "evo" || !item[i].evo) &&
      item[i].sort == undefined
    )
      continue;
    ...
    let subtitle = "";
    if (item[i].subtitle)
      subtitle = `<text style="opacity:0.6; margin-left:0.2rem">${item[i].subtitle}</text>`;

    div.dataset.item = i;
    if (item[i].type == "tm")
      div.innerHTML = `<img src="img/items/tm${format(move[item[i].move].type)}.png"> <span class="item-list-name">${format(i)} ${subtitle}<strong style="opacity:0.6; font-weight:200; white-space:nowrap; font-size:0.9rem; margin-left:0.2rem"> (${move[item[i].move].power} BP, ${format(move[item[i].move].split).slice(0, 3)})</strong> </span>  <span>x${item[i].got}</span>`;
    else if (item[i].type == "memory")
      div.innerHTML = `<img src="img/items/${item[i].image}Memory.png"> <span class="item-list-name">${format(i)} ${subtitle}</span> <span>x${item[i].got}</span>`;
    else if (item[i].type == "decor")
      div.innerHTML = `<img src="img/decor/${i}.png" style="scale:1; margin:0 -1rem"> <span class="item-list-name">${format(i)} ${subtitle}</span> <span>x${item[i].got}</span>`;
    else
      div.innerHTML = `<img src="img/items/${i}.png"> <span class="item-list-name">${format(i)} ${subtitle}</span> <span>x${item[i].got}</span>`;
...
function resetSpiralingTower() {
  ...
  while (hasDoubleWeakness) {
    selectedPokemon = randomDivisionPkmn(
      divisionToUse,
      saved.currentSpiralingType,
    );
...
function createFrontierTrainers() {
  ...
  */
    ...
    if (areas[i].tier == 1)
      areas[i].itemReward = {
        1: { item: arrayPick(exclusiveFrontierPkmn).id, amount: 1 },
      };
    if (areas[i].tier == 2)
      areas[i].itemReward = {
        1: { item: arrayPick(exclusiveFrontierPkmn).id, amount: 1 },
...
function updateFrontier() {
  ...
  if (rotationFrontierCurrent == 4)
    divisionText = `<span style="font-size: 1.5rem; padding:0">${cupsvg}Master League${cupsvg}</span><div>${returnDivisionLetter("SSS")} division and below only</div>`;
...
function loop() {
  ...
  if (saved.curry && saved.curry.time > -1) {
    ...
    document.getElementById(`curry-timer`).innerHTML =
      `<img src="img/items/friedFood.png"> ${returnHMS(saved.curry.time).replace("0h", "")}`;
  }
...
function pickArenaCard(number) {
  ...
  document
    .getElementById(`arena-card-` + number)
    .classList.add("active-arena-card");
...
function createArenaCards() {
  ...
  for (const i in field) {
    if (
      field[i].chance == undefined ||
      (field[i].chance && rng(field[i].chance))
    ) {
  ...
  if (rng(0.5))
    saved.arenaCard3 = [...arrayPick(fieldt2, 2), arrayPick(fieldt4, 1)];
}
...
function updateWildBuffs() {
  ...
  if (areas[saved.currentArea]?.fieldEffect?.includes(field.harshSun.id)) {
    saved.weather = "sunny";
    saved.weatherTimer = 5;
  }
  if (areas[saved.currentArea]?.fieldEffect?.includes(field.thickFog.id)) {
    saved.weather = "foggy";
    saved.weatherTimer = 5;
  }
  if (
    areas[saved.currentArea]?.fieldEffect?.includes(field.coarseSandstorm.id)
  ) {
    saved.weather = "sandstorm";
    saved.weatherTimer = 5;
  }
  if (areas[saved.currentArea]?.fieldEffect?.includes(field.fierceHail.id)) {
    saved.weather = "hail";
    saved.weatherTimer = 5;
  }
  if (areas[saved.currentArea]?.fieldEffect?.includes(field.heavyRain.id)) {
    saved.weather = "rainy";
    saved.weatherTimer = 5;
  }
  if (areas[saved.currentArea]?.fieldEffect?.includes(field.electricField.id)) {
    saved.weather = "electricTerrain";
    saved.weatherTimer = 5;
  }
  if (areas[saved.currentArea]?.fieldEffect?.includes(field.grassyField.id)) {
    saved.weather = "grassyTerrain";
    saved.weatherTimer = 5;
  }
  if (areas[saved.currentArea]?.fieldEffect?.includes(field.mistyField.id)) {
    saved.weather = "mistyTerrain";
    saved.weatherTimer = 5;
  }

  if (areas[saved.currentArea]?.fieldEffect?.includes(field.deltaStream.id)) {
    saved.weatherTimer = 0;
  }
...
function changeWeather(id, mod) {
  if (saved.weatherCooldown <= 0 || mod == "force") {
    ...
    if (id == "foggy" && team[exploreActiveMember].item == item.foggySeed.id)
      weatherTurns += item.foggySeed.power();
    if (
      id == "electricTerrain" &&
      team[exploreActiveMember].item == item.electricSeed.id
    )
      weatherTurns += item.electricSeed.power();
    if (
      id == "grassyTerrain" &&
      team[exploreActiveMember].item == item.grassySeed.id
    )
      weatherTurns += item.grassySeed.power();
    if (
      id == "mistyTerrain" &&
      team[exploreActiveMember].item == item.mistySeed.id
    )
      weatherTurns += item.mistySeed.power();

    if (
      /safeguard|lightScreen|crossRoom|trickRoom|weirdRoom/.test(id) &&
      team[exploreActiveMember].item == item.terrainExtender.id
    )
      weatherTurns += item.terrainExtender.power();
  ...
  for (const slot in team) {
    ...
    for (const i in team[slot].buffs) {
      ...
      if (
        ...
        team[slot].buffs.satkdown1 = 0;
        team[slot].buffs.satkdown2 = 0;
        continue;
      }
      if (
        ...
        team[slot].buffs.sdefdown1 = 0;
        team[slot].buffs.sdefdown2 = 0;
        continue;
      }

      if (
        testAbility(slot, ability.fullMetalBody.id) &&
        /atkdown1|atkdown2|defdown1|defdown2|stakdown1|satkdown2|sdefdown1|sdefdown2|spedown1|spedown2/.test(
          i,
        )
      )
        team[slot].buffs[i] = 0;
...
function moveBuff(target, buff, mod, turnOverride) {
  ...
  if (target === "player" || mod == "self" || mod == "team")
    for (const slot in team) {
      if (testAbility(slot, ability.flowerVeil.id) && buff == "paralysis")
        shouldReturn = true;
      if (testAbility(slot, ability.aromaVeil.id) && buff == "burn")
        shouldReturn = true;
      if (testAbility(slot, ability.sweetVeil.id) && buff == "confused")
        shouldReturn = true;
      if (testAbility(slot, ability.pastelVeil.id) && buff == "poisoned")
        shouldReturn = true;
    }
  ...
  if (target === "wild" && mod == "team") {
    ...
    if (
      ...
      /atkdown1|atkdown2|defdown1|defdown2|stakdown1|satkdown2|sdefdown1|sdefdown2|spedown1|spedown2|burn|freeze|confused|paralysis|poisoned|sleep/.test(
        buff,
      )
    )
      affectedTurns--;
    if (
      team[exploreActiveMember].item == item.clearAmulet.id &&
      /atkdown1|atkdown2|defdown1|defdown2|stakdown1|satkdown2|sdefdown1|sdefdown2|spedown1|spedown2|burn|freeze|confused|paralysis|poisoned|sleep/.test(
        buff,
      )
    )
      affectedTurns -= Math.max(Math.floor(item.clearAmulet.power()), 0);
    ...
    if (
      testAbility(`active`, ability.icyPelt.id) &&
      saved.weather == "hail" &&
      saved.weatherTimer > 0 &&
      /burn|freeze|confused|paralysis|poisoned|sleep|defdown1|defdown2|atkdown1|atkdown2|sdefdown1|sdefdown2|satkdown1|satkdown2|spedown1|spedown2/.test(
        buff,
    ...
    if (
      testAbility(`active`, ability.blackPelt.id) &&
      saved.weather == "fog" &&
      saved.weatherTimer > 0 &&
      /burn|freeze|confused|paralysis|poisoned|sleep|defdown1|defdown2|atkdown1|atkdown2|sdefdown1|sdefdown2|satkdown1|satkdown2|spedown1|spedown2/.test(
        buff,
    ...
    if (
      testAbility(`active`, ability.spikyPelt.id) &&
      saved.weather == "electricTerrain" &&
      saved.weatherTimer > 0 &&
      /burn|freeze|confused|paralysis|poisoned|sleep|defdown1|defdown2|atkdown1|atkdown2|sdefdown1|sdefdown2|satkdown1|satkdown2|spedown1|spedown2/.test(
        buff,
    ...
    for (const slot in team) {
      ...
      if (
        ...
        /atkup1|atkup2|defup1|defup2|satkup1|satkup2|sdefup1|sdefup2|speup1|speup2/.test(
          buff,
        )
      )
        affectedTurns++;
      if (
        team[slot].item == item.lightClay.id &&
        /atkup1|atkup2|defup1|defup2|satkup1|satkup2|sdefup1|sdefup2|speup1|speup2/.test(
          buff,
        )
      )
      ...
      if (
        testAbility(slot, ability.stoned.id) &&
        /atkup1|atkup2|defup1|defup2|satkup1|satkup2|sdefup1|sdefup2|speup1|speup2/.test(
          buff,
        )
      ) {
  ...
  if (
    ...
    //self from wild, or player to wild

    //if (buff == "paralysis" && pkmn[saved.currentPkmn].type.includes("electric")) return
    if (
      /burn|freeze|confused|paralysis|poisoned|sleep/.test(buff) &&
      Object.keys(wildBuffs).some(
        (key) =>
          /burn|freeze|confused|paralysis|poisoned|sleep/.test(key) &&
          wildBuffs[key] > 0 &&
          key !== buff,
      )
    )
    ...
    if (
      testAbility(`active`, ability.colorSpore.id) &&
      /burn|freeze|confused|paralysis|poisoned|sleep|embargo/.test(buff)
    )
      affectedTurns *= 3;
    ...
    for (const slot in team) {
      if (
        testAbility(slot, ability.imposter.id) &&
        /atkup1|atkup2|defup1|defup2|satkup1|satkup2|sdefup1|sdefup2|speup1|speup2/.test(
          buff,
        )
      )
        team[slot].buffs[buff] = 3;
    }
  ...
  if (
    ...
    //self buffs from player

    //if (buff == "paralysis" && pkmn[team[exploreActiveMember].pkmn.id].type.includes("electric")) return
    if (
      /burn|freeze|confused|paralysis|poisoned|sleep/.test(buff) &&
      Object.keys(team[exploreActiveMember].buffs || {}).some(
        (key) =>
          /burn|freeze|confused|paralysis|poisoned|sleep/.test(key) &&
          team[exploreActiveMember].buffs[key] > 0 &&
          key !== buff,
      )
    )
      return;
    ...
    if (
      ...
      return;
    if (
      testAbility(`active`, ability.stoned.id) === true &&
      /atkup1|atkup2|defup1|defup2|satkup1|satkup2|sdefup1|sdefup2|speup1|speup2/.test(
        buff,
      )
    )
      affectedTurns *= 3;
    if (
      testAbility(`active`, ability.stoned.id) === "nerf" &&
      /atkup1|atkup2|defup1|defup2|satkup1|satkup2|sdefup1|sdefup2|speup1|speup2/.test(
        buff,
      )
    )
    ...
    if (
      ...
      for (const slot in team) {
        if (testAbility(slot, ability.costar.id))
          team[slot].buffs[buff] = affectedTurns;
      }
    }
...
function changePkmnStarsign() {
  ...
  for (const i in starsign) {
    ...
    if (previewStarsignShiny)
      pkmnImg = `<img class="sprite-trim" src="img/pkmn/shiny/${id}.png">`;

    if (!pkmn[id].starsignList.includes(i)) {
      pkmnImg = `<img class="sprite-trim" style="filter:brightness(0)" src="img/pkmn/sprite/${id}.png">`;
      if (previewStarsignShiny)
        pkmnImg = `<img class="sprite-trim" style="filter:brightness(0)" src="img/pkmn/shiny/${id}.png">`;
    }
    ...
    div.addEventListener("click", (e) => {
      ...
      if (previewStarsignShiny)
        document.getElementById("pkmn-editor-sprite").src =
          `img/pkmn/shiny/${id}.png`;
      if (!previewStarsignShiny)
        document.getElementById("pkmn-editor-sprite").src =
          `img/pkmn/sprite/${id}.png`;

      document.getElementById("pkmn-editor-sprite").style.filter =
        `hue-rotate(${starsign[pkmn[id].starsign].hue}deg)`;
...
function giveStarsign(id, mode) {
  ...
  const availableStars = starList.filter(
    (star) => !pkmn[id].starsignList.includes(star),
  );
...
<img style="animation: none; position: absolute; opacity: 0.4; width: 4.5rem; z-index: 0; transform: translateY(0.5rem); image-rendering: initial;" src="img/resources/pkmn-shadow.png">`;
  ...
  if (saved.geneticHost == undefined || saved.geneticSample == undefined)
    ...
    document.getElementById("genetics-compat-text").innerHTML =
  ...
  if (item.replicatorUpgradeS.got > 0) {
    timeNeeded = Math.max(10, 7 * powerCost - 30) * 60;
  }
  const [h, m, x] = [
    (timeNeeded / 3600) | 0,
  ...
  if (pkmn[saved.geneticHost]?.hidden)
    document.getElementById("special-warning").style.display = "flex";
  ...
  if (mod === "end") {
    ...
    pkmn[saved.geneticSample].movepool.forEach((moveID) => {
      if (
        ...
        (move[moveID].moveset !== undefined ||
          (move[moveID].moveset == undefined &&
            pkmn[saved.geneticHost].eggMove?.id == moveID) ||
          (move[moveID].moveset == undefined &&
            ["B", "C", "D"].includes(
              returnPkmnDivision(pkmn[saved.geneticHost]),
            ) &&
            item.replicatorUpgradeE.got > 0 &&
            compability >= 3))
      ) {
        pkmn[saved.geneticHost].movepool.push(moveID);
        if (pkmn[saved.geneticHost].movepoolMemory == undefined)
          pkmn[saved.geneticHost].movepoolMemory = [];
        if (!pkmn[saved.geneticHost].movepoolMemory.includes(moveID))
          pkmn[saved.geneticHost].movepoolMemory.push(moveID);

        if (
          move[moveID].moveset == undefined &&
          (pkmn[saved.geneticSample].signature?.id == moveID ||
            pkmn[saved.geneticSample].eggMove?.id == moveID)
        )
          summaryTags += `<div style="filter:hue-rotate(200deg)">⟐ Egg Move inherited: ${format(moveID)}!</div>`;
        else
    ...
    pkmn[saved.geneticHost].dictionaryTagIvSum =
      pkmn[saved.geneticHost].ivs.hp +
      pkmn[saved.geneticHost].ivs.atk +
      pkmn[saved.geneticHost].ivs.satk +
      pkmn[saved.geneticHost].ivs.spe +
...
training.level = {
  ...
  effect: function () {
    ...
    if (
      pkmn[saved.trainingPokemon].evolve &&
      pkmn[saved.trainingPokemon].evolve()[1].level > 0
    ) {
      // if it evolves by level up
      if (
        pkmn[saved.trainingPokemon].level >=
          pkmn[saved.trainingPokemon].evolve()[1].level &&
        pkmn[pkmn[saved.trainingPokemon].evolve()[1].pkmn.id].caught === 0
      ) {
...
training.nature = {
  ...
  color: `#DF7A69`,
  condition: function () {
    if (areas.vsLegendTrainerBrendan.defeated == true) return true;
  },
  ...
  effect: function () {
    ...
    if (
      pkmn[saved.trainingPokemon].bst.atk <=
        pkmn[saved.trainingPokemon].bst.satk &&
      pkmn[saved.trainingPokemon].nature != "adamant" &&
      pkmn[saved.trainingPokemon].bst.atk < 6
    )
      natureList.push("adamant");
    if (
      pkmn[saved.trainingPokemon].bst.satk <=
        pkmn[saved.trainingPokemon].bst.atk &&
      pkmn[saved.trainingPokemon].nature != "modest" &&
      pkmn[saved.trainingPokemon].bst.satk < 6
    )
      natureList.push("modest");
    if (
      pkmn[saved.trainingPokemon].bst.def > 1 &&
      pkmn[saved.trainingPokemon].bst.sdef > 1 &&
      pkmn[saved.trainingPokemon].nature != "jolly" &&
      pkmn[saved.trainingPokemon].bst.spe < 6
    )
      natureList.push("jolly");
    if (
      pkmn[saved.trainingPokemon].bst.spe > 1 &&
      pkmn[saved.trainingPokemon].nature != "relaxed" &&
      pkmn[saved.trainingPokemon].bst.hp < 6
    )
      natureList.push("relaxed");
    if (
      pkmn[saved.trainingPokemon].bst.atk > 1 &&
      pkmn[saved.trainingPokemon].bst.satk > 1 &&
      pkmn[saved.trainingPokemon].nature != "quiet" &&
      pkmn[saved.trainingPokemon].bst.hp < 6
    )
      natureList.push("quiet");
    if (
      pkmn[saved.trainingPokemon].bst.def > 1 &&
      pkmn[saved.trainingPokemon].bst.sdef > 1 &&
      pkmn[saved.trainingPokemon].nature != "bold" &&
      pkmn[saved.trainingPokemon].bst.def < 6 &&
      pkmn[saved.trainingPokemon].bst.sdef < 6
    )
      natureList.push("bold");
...
function setTrainingMenu() {
  if (saved.trainingPokemon != undefined) {
    document.getElementById("training-sprite-div").innerHTML =
      `<img class="sprite-trim" style="z-index: 1; transform-origin: bottom; margin-top: auto; position: absolute; bottom: 0rem;" id="training-host" src="img/pkmn/sprite/${saved.trainingPokemon}.png">`;
    if (pkmn[saved.trainingPokemon].shiny) {
      document.getElementById("training-host").src =
        `img/pkmn/shiny/${saved.trainingPokemon}.png`;
    }
    if (pkmn[saved.trainingPokemon].starsign) {
      document.getElementById("training-host").style.filter =
        `hue-rotate(${starsign[pkmn[saved.trainingPokemon].starsign].hue}deg)`;
    }
  ...
  for (const i in training) {
    ...
    div.addEventListener("click", (e) => {
      if (
        training[i].condition &&
        training[i].condition() != true &&
        training[i].errorText
      ) {
        document.getElementById("tooltipTop").style.display = "none";
        document.getElementById("tooltipBottom").style.display = "none";
        document.getElementById("tooltipTitle").style.display = "none";
...
function debugGetPkmn(level, mod) {
  for (const i in pkmn) {
    ...
    if (mod === "starsign")
      pkmn[i].starsignList = [`sol`, `luna`, `pluto`, `ceres`, `terra`, `eris`];
  }
}
...
function testAbility(target, id) {
  if (areas[saved.currentArea]?.fieldEffect?.includes(field.neutralisingGas.id))
    return false;

  if (target == "active") {
    if (
      pkmn[team[exploreActiveMember].pkmn.id]?.ability == id &&
      ability[id].type != undefined
    )
      return true;
    if (
      pkmn[team[exploreActiveMember].pkmn.id]?.ability == id &&
      ...
      return "nerf";
    if (
      pkmn[team[exploreActiveMember].pkmn.id]?.hiddenAbility?.id == id &&
  ...
  if (/slot1|slot2|slot3|slot4|slot5|slot6/.test(target)) {
    if (
      pkmn[team[target].pkmn?.id]?.ability == id &&
      ability[id].type != undefined
    )
      return true;
    if (
      ...
      return "nerf";
    if (
      pkmn[team[target].pkmn?.id]?.hiddenAbility?.id == id &&
  ...
  if (
    areas[saved.currentArea]?.fieldEffect?.includes(field.simpleAura.id) &&
    id == ability.simple.id
  )
    return true;
  if (
    areas[saved.currentArea]?.fieldEffect?.includes(field.moodyAura.id) &&
    id == ability.moody.id
  )
    return true;
  ...
  if (saved.curry?.time > 0 && saved.curry?.effect.includes(id)) {
    if (
      (areas[saved.currentArea]?.type == `event` &&
        areas[saved.currentArea]?.difficulty >= tier1difficulty) ||
      (areas[saved.currentArea]?.type == `dimension` &&
        areas[saved.currentArea]?.difficulty >= tier1difficulty)
    ) {
      return true;
...
function claimWonderTrade() {
  ...
  if (areas.vsMasterTrainerGeeta.defeated == false) {
    ...
    document.getElementById("tooltipBottom").style.display = `none`;
    document.getElementById("tooltipMid").innerHTML =
      `Defeat Master Trainer Geeta in VS mode to unlock`;
    openTooltip();
    return;
  }
  ...
  document.getElementById("tooltipMid").innerHTML =
    `Every 12h you might receive a random pokemon`;
  document.getElementById("tooltipBottom").innerHTML = `
...
function wonderTrade() {
  ...
  for (const i in pkmn) {
    if (pkmn[i].caught > 0) continue;
    if (
      pkmn[i].tagObtainedIn == "frontier" ||
      pkmn[i].tagObtainedIn == "wild" ||
      pkmn[i].tagObtainedIn == "park"
    )
      unobtainedPokemon.push(i);
  }

  if (rng(0.5) && unobtainedPokemon.length > 0) {
    //new pokemon
  ...
  document.getElementById("wonder-text").innerHTML =
    `Thanks for the ${format(chosenPokemon)}!`;
  document.getElementById("wonder-pkmn").src =
    `img/pkmn/sprite/${chosenPokemon}.png`;
  if (chosenShiny)
    document.getElementById("wonder-pkmn").src =
      `img/pkmn/shiny/${chosenPokemon}.png`;
...
function arceusCheck() {
  if (pkmn.arceus.movepool.length == 0)
    pkmn.arceus.movepool.push(move.judgment.id);
  ...
  if (statPkmnGot == statPkmnTotal) {
    ...
    for (const i in saved.previewTeams) {
      ...
      for (const st in saved.previewTeams[i]) {
        if (saved.previewTeams[i][st].pkmn == pkmn.arceus.id)
          saved.previewTeams[i][st].pkmn = undefined;
      }
    }
  }
...
function seasonalSwitch() {
  ...
  document.getElementById("tooltipMid").innerHTML =
    `Select the seasonal event to start. It will last for the 10 following days, and will get replaced by currently-ongoing ones. However, the seasonal shop of that event wont open`;
  document.getElementById("tooltipBottom").innerHTML =
    `<div id="seasonal-pick-listing"></div>`;
  openTooltip();
...
function seasonCheck() {
  ...
  if (saved.temporalSeason) {
    const endDate =
      saved.temporalSeason.end.month * 100 + saved.temporalSeason.end.day;
  ...
  if (newSeason) {
    //executes once per season, resets shop
    console.info(
      `${format(saved.currentSeason)} season started, shops restocked!`,
    );
  ...
  if (saved.halloweenThemeUnlocked == true)
...
function renamePokemon() {
  ...
  document
    ...
    .addEventListener("keydown", (e) => {
      if (e.key === "Enter") {
        pkmn[currentEditedPkmn].nickname =
          document.getElementById("team-name-field").value;

        if (
          currentEditedPkmn == pkmn.onix.id &&
          ["crystal", "Crystal", "crystal onix", "Crystal Onix"].includes(
            document.getElementById("team-name-field").value,
          )
        ) {
        ...
        document.getElementById("pkmn-editor-nickname").textContent =
          pkmn[currentEditedPkmn].nickname;
...
function assignMegaDimension() {
  ...
  if (saved.lastDimensionRotation != rotationEventCurrent) {
    saved.lastDimensionRotation = rotationEventCurrent;
  }
  ...
  areas.dimensionRift1.spawns = {
    common: [
      pkmn[randomDivisionPkmn("S", arrayPick(allTypes))],
      pkmn[randomDivisionPkmn("S", arrayPick(allTypes))],
      pkmn[randomDivisionPkmn("S", arrayPick(allTypes))],
    ],
  };
...
function updateMegaDimension(tier) {
  ...
  if (tier == undefined) {
    ...
    for (const i in areas) {
      ...
      if (areas[i].tier == 2)
        dimensionIndicator = `<div style="filter:hue-rotate(0deg)" id="dimension-indicator">★★</div>`;
      if (areas[i].tier == 3)
        dimensionIndicator = `<div style="filter:hue-rotate(200deg)" id="dimension-indicator">★★★</div>`;
      if (areas[i].tier == 4)
        dimensionIndicator = `<div style="filter:hue-rotate(300deg)" id="dimension-indicator">★★★★</div>`;
    ...
    // update the areas on themselves
    ...
    document.getElementById("dimension-listing").innerHTML +=
      `<div class="dimension-return" onclick="updateMegaDimension()">
