...
shop.shopepochFeather = {
  ...
  category: `goods`,
  condition: function () {
    if (rotationEventCurrent == 1) return true;
  },
  effect: function () {
    item.epochFeather.got += 25;
  },
};
...
shop.shopancientOrchid = {
  ...
  category: `goods`,
  condition: function () {
    if (rotationEventCurrent == 2) return true;
  },
  effect: function () {
    item.ancientOrchid.got += 25;
  },
};
...
shop.shopancientKeystone = {
  ...
  category: `goods`,
  condition: function () {
    if (rotationEventCurrent == 3) return true;
  },
  effect: function () {
    item.ancientKeystone.got += 25;
  },
};
...
shop.shopaetherKeycard = {
  ...
  category: `goods`,
  condition: function () {
    if (rotationEventCurrent == 4) return true;
  },
  effect: function () {
    item.aetherKeycard.got += 25;
  },
};
...
shop.shopfutureDisk = {
  ...
  category: `goods`,
  condition: function () {
    if (rotationEventCurrent == 5) return true;
  },
  effect: function () {
    item.futureDisk.got += 25;
  },
};
...
shop.shopwisdomPetal = {
  ...
  category: `goods`,
  condition: function () {
    if (rotationEventCurrent == 6) return true;
  },
  effect: function () {
...
shop.shopgalarianZigzagoon = {
  ...
  category: `pokemon`,
  effect: function () {
    givePkmn(pkmn.galarianZigzagoon, 1);
  },
};
...
shop.shopgalarianPonyta = {
  ...
  category: `pokemon`,
  effect: function () {
    givePkmn(pkmn.galarianPonyta, 1);
  },
};
...
shop.shopalolanSandshrew = {
  ...
  category: `pokemon`,
  effect: function () {
    givePkmn(pkmn.alolanSandshrew, 1);
  },
};
...
shop.shophisuianVoltorb = {
  ...
  category: `pokemon`,
  effect: function () {
    givePkmn(pkmn.hisuianVoltorb, 1);
  },
};
...
shop.shophisuianQwilfish = {
  ...
  category: `pokemon`,
  effect: function () {
    givePkmn(pkmn.hisuianQwilfish, 1);
  },
};
...
shop.shophisuianBraviary = {
  ...
  category: `pokemon`,
  effect: function () {
    givePkmn(pkmn.hisuianBraviary, 1);
  },
};
...
shop.shophisuianSamurott = {
  ...
  category: `pokemon`,
  effect: function () {
    givePkmn(pkmn.hisuianSamurott, 1);
  },
};
...
shop.shophisuianLilligant = {
  ...
  category: `pokemon`,
  effect: function () {
    givePkmn(pkmn.hisuianLilligant, 1);
  },
};
...
shop.shopvivillonElegant = {
  ...
  category: `pokemon`,
  effect: function () {
    givePkmn(pkmn.vivillonElegant, 1);
  },
};
...
shop.eventhalloweenTheme = {
  ...
  category: `limited`,
  effect: function () {
    saved.halloweenThemeUnlocked = true;
  },
  condition: function () {
    if (saved.currentSeason == season.halloween.id) return true;
  },
};
...
shop.shopApricornBlackItem3 = {
  pkmn: pkmn.kyuremBlack.id,
  name: `Kyurem-Black`,
  price: 999,
  currency: item.blackApricorn.id,
  category: `apricorn`,
  effect: function () {
    givePkmn(pkmn.kyuremBlack, 1);
  },
};
...
function assignShopApricorn() {
  if (
    ...
    !Array.isArray(saved.shopApricornMemoryRotation) ||
    !Array.isArray(saved.shopApricornMemoryRotationWhite) ||
    !Array.isArray(saved.shopApricornMemoryRotationBlack)
  ) {
    saved.lastShopApricornReset = rotationWildCurrent;
...
function assignShopDecor() {
  if (saved.lastShopDecorReset != rotationEventCurrent) {
    ...
    saved.shopDecorRotation = [
      ...arrayPick(commonDecor, 5),
      arrayPick(rareDecor, 1),
    ];
  }
...
function updateItemShop() {
  ...
  if (areas.vsLegendTrainerBrendan.defeated == false) {
    document.getElementById("shop-apricorn-exchange").style.filter =
      "brightness(0.3)";
  ...
  if (areas.vsLegendTrainerBrendan.defeated == false) {
    ...
    document.getElementById("shop-restaurant").style.filter = "brightness(1)";
  ...
  for (let i in shop) {
    ...
    if (shop[i].icon) {
      ...
      if (
        (item[shop[i].icon].type == "held" &&
          item[shop[i].icon].got >= 20 &&
          item[shop[i].icon].heldBonusPkmn == undefined) ||
        (item[shop[i].icon].heldBonusPkmn &&
          pkmn[item[shop[i].icon].heldBonusPkmn()].caught == 0 &&
          item[shop[i].icon].got >= 21) ||
        (item[shop[i].icon].heldBonusPkmn &&
          pkmn[item[shop[i].icon].heldBonusPkmn()].caught > 0 &&
          item[shop[i].icon].got >= 20)
      )
      ...
      if (item[shopItem] && item[shopItem].type == "tm") {
        div.innerHTML = `
    ...
    `;
      } else if (item[shopItem] && item[shopItem].type == "memory") {
        div.innerHTML = `
    ...
    `;
      } else if (item[shopItem] && item[shopItem].type == "decor") {
        div.innerHTML = `
    ...
    function buyItem(amount) {
      if (
        ...
        document.getElementById("tooltipMid").style.display = "none";
        document.getElementById("tooltipBottom").innerHTML =
          `You cant afford to purchase this<span id="prevent-tooltip-exit"></span>`;
      }
    }
...
function updateCurry() {
  if (saved.lastCurryRotation == rotationWildCurrent) {
    ...
    document.getElementById(`curry-title`).innerHTML =
      `Come back in <span class="time-counter-daily"></span>`;
    updateDailyCounters();
    return;
  } else {
  ...
  document.getElementById(`curry-title`).innerHTML =
    `Select ingredients to add to the curry! [${curryIngredientList.length}/3]`;
  ...
  document.getElementById(`curry-pay`).innerHTML =
    `Pay and cook (${totalPrice} <img src="img/items/goldenBottleCap.png">)`;
...
function makeCurry() {
  ...
  document.getElementById(`curry-ingredients`).style.animation =
    `curry-drop 0.8s ease-in-out`;

  setTimeout(() => {
    document.getElementById(`curry-pot`).style.animation =
      `itemShake 0.4s ease-in-out`;
    document.getElementById(`curry-ingredients`).innerHTML = ``;
  }, 700);
