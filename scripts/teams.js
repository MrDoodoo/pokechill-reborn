...
function injectPreviewTeam() {
  ...
  if (
    saved.previewTeams[saved.currentPreviewTeam].slot1.pkmn == pkmn.unownG.id &&
  ...
  for (const i in currentTeam) {
    ...
    if (
      ...
      pkmn[currentTeam[i].pkmn].nuzlocked == true
    )
      nuzlockeError = true;
  }
  ...
  if (zCrystalError > 1) {
    ...
    document.getElementById("tooltipMid").innerHTML =
      `Multiple Z-Crystals cannot be equiped in a single team`;
    openTooltip();
    return;
  }
  ...
  for (const i in currentTeam) {
    ...
    for (const activeMoves in pkmn[currentTeam[i].pkmn].moves) {
      if (pkmn[currentTeam[i].pkmn].moves[activeMoves] == undefined) continue;
      if (pkmn[currentTeam[i].pkmn].moves[activeMoves] == move.batonPass.id)
        batonPassCount++;
    }
  ...
  if (batonPassError) {
    ...
    document.getElementById("tooltipMid").innerHTML =
      `You can only have one Baton Pass in your team`;
    openTooltip();
    return;
  }
  ...
  for (const i in currentTeam) {
    ...
    for (const activeMoves in pkmn[currentTeam[i].pkmn].moves) {
      ...
      if (
        pkmn[currentTeam[i].pkmn].signature?.id ==
        pkmn[currentTeam[i].pkmn].moves[activeMoves]
      )
        continue;
      if (
        pkmn[currentTeam[i].pkmn].eggMove?.id ==
        pkmn[currentTeam[i].pkmn].moves[activeMoves]
      )
        continue;

      if (eggMoveList.includes(pkmn[currentTeam[i].pkmn].moves[activeMoves]))
        eggMoveError = true;

      if (
        move[pkmn[currentTeam[i].pkmn].moves[activeMoves]].moveset == undefined
      )
        eggMoveList.push(pkmn[currentTeam[i].pkmn].moves[activeMoves]);
    }
  }
  ...
  if (eggMoveError) {
    ...
    document.getElementById("tooltipMid").innerHTML =
      `Multiple of the same Egg Move cannot exist on the same team if the host does not have said move as an inherent Egg Move or Signature Move`;
    openTooltip();
    return;
  }
...
function switchMemberNext(direction = 1) {
  // -1 for backwards
...
function switchMember(member) {
  ...
  if (pkmn[team[exploreActiveMember].pkmn.id].playerHp > 0) {
    ...
    if (
      document
        .getElementById(`explore-team-member-${exploreActiveMember}-spriteData`)
...
function setPkmnTeam() {
  ...
  for (const i in team) {
    ...
    div.addEventListener("click", (e) => {
      ...
      if (
        areas[saved.currentArea].fieldEffect?.includes(field.stealthRocks.id)
      ) {
        if (
          team[i].item != item.heavyDutyBoots.id &&
          i != exploreActiveMember
        ) {
          pkmn[team[i].pkmn.id].playerHp -=
            pkmn[team[i].pkmn.id].playerHpMax / 5;
          updateTeamPkmn();
        }
      }
      ...
      if (saved.currentArea == areas.frontierBattleFactory.id) {
        pkmn[team[exploreActiveMember].pkmn.id].playerHp -=
          pkmn[team[exploreActiveMember].pkmn.id].playerHpMax / 15;
        updateTeamPkmn();
      }
    });
    ...
    if (pkmn[team[i].pkmn.id].nickname)
      nickname = pkmn[team[i].pkmn.id].nickname;
    ...
    if (pkmn[team[i].pkmn.id].shiny)
      pkmnName = `${nickname} <span style="color:#FF4671;">✦</span> <span class="explore-pkmn-level" id="explore-${i}-lvl">lvl ${pkmn[team[i].pkmn.id].level}</span>`;
    ...
    if (pkmn[team[i].pkmn.id].starsign) {
      ...
      if (pkmn[team[i].pkmn.id].shiny)
        pkmnSprite = `<img style="filter:hue-rotate(${starsign[pkmn[team[i].pkmn.id].starsign].hue}deg)" class="sprite-trim" src="img/pkmn/shiny/${team[i].pkmn.id}.png" id="explore-team-member-${i}-sprite">`;
      if (
        pkmn[team[i].pkmn.id].shiny &&
        pkmn[team[i].pkmn.id].shinyDisabled == true
      )
    ...
    for (const e in pkmn[team[i].pkmn.id].moves) {
      ...
      if (moveId == pkmn[team[i].pkmn.id].signature?.id)
        signatureIcon = `<svg style="color:${returnTypeColor(move[moveId].type)}" xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24"><path fill="currentColor" d="M21.951 9.67a1 1 0 0 0-.807-.68l-5.699-.828l-2.548-5.164A.98.98 0 0 0 12 2.486v16.28l5.097 2.679a1 1 0 0 0 1.451-1.054l-.973-5.676l4.123-4.02a1 1 0 0 0 .253-1.025" opacity="0.5"/><path fill="currentColor" d="M11.103 2.998L8.555 8.162l-5.699.828a1 1 0 0 0-.554 1.706l4.123 4.019l-.973 5.676a1 1 0 0 0 1.45 1.054L12 18.765V2.503a1.03 1.03 0 0 0-.897.495"/></svg>`;
...
function updatePreviewTeam() {
  ...
  for (const i in currentTeam) {
    ...
    if (currentTeam[i].pkmn !== undefined) {
      ...
      if (pkmn[currentTeam[i].pkmn].nickname)
        nickname = pkmn[currentTeam[i].pkmn].nickname;
      ...
      if (pkmn[currentTeam[i].pkmn].shiny)
        pkmnName = `${nickname} ${nameTag} <span style="color:#FF4671;">✦</span> <span class="explore-pkmn-level" id="explore-${i}-lvl">lvl ${pkmn[currentTeam[i].pkmn].level}</span>`;
      ...
      if (pkmn[currentTeam[i].pkmn].starsign) {
        ...
        if (pkmn[currentTeam[i].pkmn].shiny)
          pkmnSprite = `<img style="filter:hue-rotate(${starsign[pkmn[currentTeam[i].pkmn].starsign].hue}deg)" class="sprite-trim" src="img/pkmn/shiny/${currentTeam[i].pkmn}.png" id="explore-team-member-${i}-sprite">`;
        if (
          pkmn[currentTeam[i].pkmn].shiny &&
          pkmn[currentTeam[i].pkmn].shinyDisabled == true
        )
      ...
      for (const e in pkmn[currentTeam[i].pkmn].moves) {
        ...
        if (move[moveId].moveset == undefined)
          signatureIcon = `<svg style="color:${returnTypeColor(move[moveId].type)}" xmlns="http://www.w3.org/2000/svg" width="48" height="48" viewBox="0 0 48 48"><defs><mask id="eggMask${moveId}"><g fill="none"><path fill="#555555" fill-rule="evenodd" stroke="#fff" stroke-width="4" d="M24 44c10.252 0 16-6.954 16-18S31.132 4 24 4S8 14.954 8 26s5.748 18 16 18Z" clip-rule="evenodd"/><path fill="#fff" d="M21 38a3 3 0 1 0 0-6a3 3 0 0 0 0 6"/><path fill="#fff" fill-rule="evenodd" d="M16 29.668a2 2 0 1 0 0-4a2 2 0 0 0 0 4" clip-rule="evenodd"/></g></mask></defs><path fill="currentColor" d="M0 0h48v48H0z" mask="url(#eggMask${moveId})"/></svg>`;
        if (moveId == pkmn[pkmn[currentTeam[i].pkmn].id].signature?.id)
          signatureIcon = `<svg style="color:${returnTypeColor(move[moveId].type)}" xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24"><path fill="currentColor" d="M21.951 9.67a1 1 0 0 0-.807-.68l-5.699-.828l-2.548-5.164A.98.98 0 0 0 12 2.486v16.28l5.097 2.679a1 1 0 0 0 1.451-1.054l-.973-5.676l4.123-4.02a1 1 0 0 0 .253-1.025" opacity="0.5"/><path fill="currentColor" d="M11.103 2.998L8.555 8.162l-5.699.828a1 1 0 0 0-.554 1.706l4.123 4.019l-.973 5.676a1 1 0 0 0 1.45 1.054L12 18.765V2.503a1.03 1.03 0 0 0-.897.495"/></svg>`;
